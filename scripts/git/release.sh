...
tag_repo() {
    ...
    if [ -n "$SUB_MODULE" ]; then
        ...
        docker tag azureiotpcs/$REPO_NAME$DEFAULT_TAG  azureiotpcs/$REPO_NAME:$VERSION
    else
        echo "Building reverse_proxy..."
        cd $APP_HOME/reverse-proxy
        /bin/bash build
        echo -e "Tagging azureiotpcs/remote-monitoring-nginx:$VERSION"
        docker tag azureiotpcs/remote-monitoring-nginx$DEFAULT_TAG      azureiotpcs/remote-monitoring-nginx:$VERSION
    fi
...
# Java Microservices
# Only have dotnet service for auth
# tag_repo auth               pcs-auth-java
tag_repo config             pcs-config-java
# Only have dotnet service for device simulation
# tag_repo device-simulation  device-simulation-dotnet
tag_repo iothub-manager     iothub-manager-java
tag_repo storage-adapter    pcs-storage-adapter-java
tag_repo telemetry          device-telemetry-java
tag_repo telemetry-agent    telemetry-agent-java
# Doing this as part of Dotnet release
...
# Top Level repo
tag_repo ""                 azure-iot-pcs-remote-monitoring-java $DESCRIPTION
